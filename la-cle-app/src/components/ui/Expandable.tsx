...
export function Expandable({ title, children, defaultOpen = false }: ExpandableProps) {
  ...
  return (
    <div className="border-b border-filet">
      ...
      >
        <span className="pr-8 font-serif text-lg text-ivoire">{title}</span>
        <span
          className={`shrink-0 text-lg text-or transition-transform duration-500 ${
            isOpen ? "rotate-45" : ""
          }`}
        >
          +
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="pb-6 leading-relaxed text-cendre">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
