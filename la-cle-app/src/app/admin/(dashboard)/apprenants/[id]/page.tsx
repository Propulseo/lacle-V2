...
import type { Learner, Document } from "@/types";
...
export default function ApprenantDetailPage() {
  ...
  async function handleToggleActive() {
    if (!learner) return;
    const updated = await toggleActive(learner.id);
    setLearner(updated);
  }
        ...
        <Card>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              <Badge variant={learner.status === "certifie" ? "gold" : learner.status === "valide" ? "success" : learner.status === "en_cours" ? "info" : "error"}>
                {statusCfg.label}
              </Badge>
                ...
                <Card key={doc.id}>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-cendre">{doc.fileName} • {formatDate(doc.uploadedAt)}</p>
                    </div>
