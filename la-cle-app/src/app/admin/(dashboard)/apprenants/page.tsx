...
import { STATUS_CONFIG, type LearnerStatusType } from "@/lib/status";
  ...
  const columns: Column<Learner>[] = [
    {
      ...
      render: (l) => (
        <div>
          ...
          <p className="text-xs text-cendre">{l.email}</p>
        </div>
      ),
    },
    {
      ...
      render: (l) => {
        ...
        return <Badge variant={l.status === "certifie" ? "gold" : l.status === "valide" ? "success" : l.status === "en_cours" ? "info" : "error"}>{cfg.label}</Badge>;
      },
    },
    {
      key: "progression",
      header: "Progression",
      render: (l) => (
        <ProgressBar value={l.progression.overallPercent} showLabel size="sm" />
      ),
      className: "w-48",
    },
    {
    ...
    },
    {
      ...
      header: "Actif",
      render: (l) => (
        <span className={`h-2 w-2 rounded-full inline-block ${l.isActive ? "bg-succes" : "bg-erreur"}`} />
      ),
      className: "w-16",
    },
  ];
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-cendre">{learners.length} inscrits</p>
          </div>
          <Button
            variant="primary"
            icon={<Plus className="h-4 w-4" />}
            href={ROUTES.admin.nouveauApprenant}
          >
            Nouveau
          </Button>
        </div>
