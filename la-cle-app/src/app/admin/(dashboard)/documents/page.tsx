...
import { FileText, MessageSquare, Plus, Reply } from "lucide-react";
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    getSupportMessages().then(setMessages);
  }, []);
  ...
  const filteredDocs = search
    ? documents.filter(
        (d) =>
          ...
          d.learnerName.toLowerCase().includes(search.toLowerCase())
      )
    : documents;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-cendre">
              {documents.length} documents • {unreplied.length} message(s) en attente
            </p>
          </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-cendre">
                            {doc.learnerName} • {doc.fileName} • {formatDate(doc.uploadedAt)}
                          </p>
                        </div>
                      </div>
                ...
                <Card>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-xs text-cendre">{msg.learnerName} • {formatDate(msg.createdAt)}</p>
                    </div>
                    <Badge variant={msg.reply ? "success" : "warning"}>
                  ...
                  {msg.reply && (
                    <div className="mt-3 rounded-lg bg-surface p-3">
                      <p className="text-xs text-pierre mb-1 flex items-center gap-1">
                  ...
                  {!msg.reply && (
                    <div className="mt-3">
                      <Button size="sm" icon={<Reply className="h-4 w-4" />}>
                        Répondre
                      </Button>
                    </div>
                  )}
